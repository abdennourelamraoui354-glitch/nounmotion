...
import { BUSINESS, TEAL, DARK, ACCENT } from '../lib/business-data'
      ...
      {particles.map((p) => (
        <motion.div
          ...
          style={{
            ...
            width: p.size,
            height: p.size,
            background: p.id % 4 === 0 ? ACCENT : TEAL,
          }}
          animate={{
            y: [0, -30, 0],
...
export function HeroReel() {
  ...
  const waHref = `https://wa.me/${BUSINESS.whatsapp.replace(/\D/g, '')}?text=${encodeURIComponent("السلام عليكم! أريد حجز استشارة في عيادة المجد لطب الأسنان.")}`
    ...
    <section className="relative min-h-screen flex flex-col justify-center overflow-hidden"
      ...
      <nav className="absolute top-0 left-0 right-0 flex items-center justify-between px-8 lg:px-16 py-6 z-20">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 2.2, duration: 0.6 }}>
          <span className="text-sm font-light tracking-[0.35em] uppercase" style={{ color: ACCENT }}>المجد</span>
          ...
          <div className="flex flex-col gap-8">
            ...
            {ready && (
              <motion.div
                ...
                <span className="text-[10px] font-medium tracking-[0.4em] uppercase" style={{ color: ACCENT }}>
                  {BUSINESS.neighborhood} · Muscat, Oman
                </span>
              </motion.div>
            )}
            ...
            {ready && (
              <motion.div
                ...
                {[
                  ...
                  { v: 'Ghubra', l: 'Muscat, Oman' },
                ].map(stat => (
                  <div key={stat.l}>
                    <p className="text-2xl font-black" style={{ color: ACCENT }}>{stat.v}</p>
