...
const FALLBACK = BUSINESS.menu[0].img;

export function MenuBento() {
  const ref = useRef<HTMLDivElement>(null);
  const inView = useInView(ref, { once: true, margin: '-80px' });
  const waHref = `https://wa.me/${BUSINESS.whatsapp.replace(/\D/g, '')}?text=${encodeURIComponent('مرحباً، أود الاستفسار عن القائمة')}`;
          ...
          {BUSINESS.menu.map((item, i) => {
            ...
            return (
              <motion.div
                ...
                style={{ minHeight: isLarge ? 420 : 200, background: '#111', border: `1px solid ${GOLD}18` }}
              ...
              >
                ...
                <div className="absolute top-4 right-4">
                  <span
                    ...
                    style={{ background: `${DARK_BG}CC`, backdropFilter: 'blur(12px)', color: GOLD, fontFamily: 'Tajawal, sans-serif', border: `1px solid ${GOLD}40` }}
                  >
                    {item.category}
                  </span>
                </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-5">
                  <div className="flex items-end justify-between gap-2">
                    <div className="flex-1">
                      <p className="text-lg font-semibold leading-tight mb-1" style={{ fontFamily: 'Tajawal, sans-serif', color: CREAM, direction: 'rtl' }}>
                        {item.nameAr}
                      </p>
                      <p className="text-sm mb-2" style={{ fontFamily: 'Cormorant Garamond, serif', color: `${GOLD}CC`, fontStyle: 'italic' }}>
                        {item.nameEn}
                      </p>
                      {isLarge && (
                        <p className="text-xs leading-relaxed mb-3 hidden md:block" style={{ color: `${CREAM}70`, fontFamily: 'DM Sans, sans-serif', direction: 'rtl', textAlign: 'right' }}>
                          {item.descAr}
                        </p>
                      )}
                    </div>
                    <div className="flex flex-col items-end gap-2 shrink-0">
                      <span className="font-bold text-lg" style={{ fontFamily: 'Cormorant Garamond, serif', color: GOLD }}>
                        {item.price}
                      </span>
                      {isLarge && (
                        <a
