    ...
    <section id="story" ref={containerRef} className="relative overflow-hidden md:h-[100dvh]" style={{ background: DARK_BG }}>
      <div ref={bgRef} className="absolute inset-0 origin-center hidden md:block">
        <div
          ...
          style={{ backgroundImage: `url(${BUSINESS.storyBg})`, backgroundSize: 'cover', backgroundPosition: 'center' }}
        />
        <div className="absolute inset-0" style={{ background: 'rgba(10,9,8,0.84)' }} />
      </div>
        ...
        <div className="relative w-full max-w-2xl md:h-[300px] flex flex-col gap-16 md:block md:gap-0">
          {BUSINESS.story.map((chapter, i) => (
            <div
              ...
              style={{ opacity: i === 0 ? 1 : undefined, background: 'rgba(255,255,255,0.03)', border: `1px solid ${GOLD}18` }}
            >
              <span
              ...
              <p className="text-sm md:text-lg max-w-lg leading-relaxed" style={{ color: `${CREAM}80`, fontFamily: 'DM Sans, sans-serif' }}>
                {chapter.textEn}
              </p>
              <p className="mt-3 text-sm md:text-base" style={{ fontFamily: 'Tajawal, sans-serif', color: `${GOLD}99`, direction: 'rtl' }}>
                {chapter.textAr}
              </p>
            </div>
          ))}
        ...
        <div className="flex gap-3 mt-10 md:mt-12">
          {BUSINESS.story.map((_, i) => (
            <div
              ...
              style={{ width: i === activePanel ? 24 : 8, height: 8, background: i === activePanel ? GOLD : `${CREAM}30` }}
            />
          ))}
        </div>
      </div>
