      ...
      >
        ...
        >
          <img
            ...
            loading="eager"
            decoding="async"
            onError={(e) => { (e.currentTarget as HTMLImageElement).src = BUSINESS.heroSlides[0].img; }}
          />
        </div>

        <style>{`
          @keyframes mrsteam1 { 0%,100%{transform:translateY(0) scaleX(1);opacity:0.6} 50%{transform:translateY(-18px) scaleX(1.4);opacity:0} }
          @keyframes mrsteam2 { 0%,100%{transform:translateY(0) scaleX(1);opacity:0.4} 50%{transform:translateY(-22px) scaleX(0.8);opacity:0} }
        `}</style>
          ...
          {[0, 0.3, 0.6].map((d, i) => (
            <div
              ...
              style={{
                ...
                borderRadius: 4,
                background: `${GOLD}80`,
                animation: `mrsteam${i % 2 === 0 ? 1 : 2} ${2.2 + i * 0.3}s ${d}s ease-in-out infinite`,
              }}
            />
          ))}
        </div>
