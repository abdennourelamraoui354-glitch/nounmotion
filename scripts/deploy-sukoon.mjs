#!/usr/bin/env node
/**
 * Deploy Sukoon Restaurant → sukoon.nounmotion.store
 * Usage: node scripts/deploy-sukoon.mjs
...
const SERVICE = 'sukoon';
const DOMAIN = 'sukoon.nounmotion.store';
const EP_HOST = 'nounmotion-sukoon.znnri1.easypanel.host';
...
async function main() {
  ...
  if (dirty) {
    run(
      'git -c user.email="abdennourelamraoui354@gmail.com" -c user.name="abdennourelamraoui354-glitch" commit -m "Deploy Sukoon Restaurant to sukoon.nounmotion.store"',
    );
  }
  ...
  console.log('4/6 Create sukoon service (if missing)...');
    ...
    await trpc(
      'services.app.createService',
      {
        json: {
          ...
          build: { type: 'dockerfile', file: 'Dockerfile.sukoon' },
          env: '',
          domains: [
  ...
  console.log('6/6 Cloudflare worker...');
  run('npx wrangler deploy --config wrangler-sukoon.toml');

  console.log('\nWaiting 90s for build...');
