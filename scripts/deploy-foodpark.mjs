#!/usr/bin/env node
/**
 * Deploy Foodpark Asian Restaurant → foodpark.nounmotion.store
 * Usage: node scripts/deploy-foodpark.mjs
...
const SERVICE = 'foodpark';
const DOMAIN = 'foodpark.nounmotion.store';
const EP_HOST = 'nounmotion-foodpark.znnri1.easypanel.host';
...
async function main() {
  ...
  console.log('2/6 Git commit + push...');
  run('git add -A');
  const dirty = execSync('git status --porcelain', { encoding: 'utf8' }).trim();
  if (dirty) {
    run(
      'git -c user.email="abdennourelamraoui354@gmail.com" -c user.name="abdennourelamraoui354-glitch" commit -m "Deploy Foodpark Asian Restaurant to foodpark.nounmotion.store"',
    );
  }
  ...
  console.log('4/6 Create foodpark service (if missing)...');
    ...
    await trpc(
      'services.app.createService',
      {
        json: {
          ...
          build: { type: 'dockerfile', file: 'Dockerfile.foodpark' },
          env: '',
          domains: [
  ...
  console.log('5/6 Deploy EasyPanel...');
  await trpc(
  ...
  console.log('6/6 Cloudflare worker deploy...');
  run('npx wrangler deploy --config wrangler-foodpark.toml');

  console.log('\n⏳ Waiting 90s for build to complete...');
