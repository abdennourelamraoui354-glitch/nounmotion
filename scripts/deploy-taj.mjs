#!/usr/bin/env node
/** Deploy Taj Restaurant → taj.nounmotion.store */
import { execSync } from 'node:child_process';

const EP = process.env.EASYPANEL_URL || 'http://76.13.43.170:3000';
const PROJECT = 'nounmotion';
const SERVICE = 'taj';
const DOMAIN = 'taj.nounmotion.store';
const EP_HOST = 'nounmotion-taj.znnri1.easypanel.host';
...
async function main() {
  ...
  if (dirty) {
    run(
      'git -c user.email="abdennourelamraoui354@gmail.com" -c user.name="abdennourelamraoui354-glitch" commit -m "Deploy Taj Restaurant to taj.nounmotion.store"',
    );
  }
  ...
  console.log('4/6 Create taj service (if missing)...');
    ...
    await trpc(
      'services.app.createService',
      {
        json: {
          ...
          build: { type: 'dockerfile', file: 'Dockerfile.taj' },
          env: '',
          domains: [
  ...
  run('npx wrangler deploy --config wrangler-taj.toml');
  ...
  console.log(`\nLive: https://${DOMAIN}/`);
  console.log(`Backup: https://nounmotion.store/taj`);
}
